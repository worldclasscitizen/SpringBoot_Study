...
class CommentRepositoryTest {
    ...
    void findByArticleId() {
        ...
        }
        {
            ...
            List<Comment> expected = Arrays.asList();
            // 4. 비교 및 검증
            assertEquals(comments, expected, "9번 게시글 자체가 없으므로 예상 데이터가 없다.");
        }
        {
            ...
            List<Comment> expected = Arrays.asList();
            // 4. 비교 및 검증
            assertEquals(comments, expected, "999번 게시글 자체가 없으므로 예상 데이터가 없다.");
        }
        {
            ...
            List<Comment> expected = Arrays.asList();
            // 4. 비교 및 검증
            assertEquals(comments, expected, "-1번 게시글 자체가 없으므로 예상 데이터가 없다.");
        }
    }
    ...
    void findByNickname() {
        ...
        }
        {
            ...
            List<Comment> expected = Arrays.asList();
            // 4. 비교 및 검증
            assertEquals(expected, comments, "닉네임 null 은 잘못된 입력이다!");
        }
        {
            /* Case 4 : "" 의 모든 댓글 조회 */
            // 1. 입력 데이터 준비
            ...
            List<Comment> expected = Arrays.asList();
            // 4. 비교 및 검증
            assertEquals(expected, comments, "비어 있는 닉네임은 잘못된 입력이다!");
        }
    }
}